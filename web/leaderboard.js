...
function ROW_HTML(r){
  return `
    <tr>
      ...
      <td class='right'>${r.robustness!==undefined?(r.robustness*100).toFixed(1)+'%':'—'}</td><td class='right'>${r.bias!==undefined?(r.bias*100).toFixed(1)+'%':'—'}</td><td class='right'>${r.calibration!==undefined?(r.calibration*100).toFixed(1)+'%':'—'}</td>
